...
exports.pageviews = async (req, res) => {
  try {
    const alldata = await user.find()
      .populate({path: 'project',populate: [ { path: 'department' }, 
          { path: 'employees',populate: { path: 'department' }} ]}).populate({path :'assignedTo',populate :{path:'department',}}) .populate('department');
    ...
    // populate('project').populate('assignedTo').populate('department')
    console.log("=======", alldata)
    res.status(200).json({
      status: 'success',
